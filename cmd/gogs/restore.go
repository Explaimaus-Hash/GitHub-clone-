...
The backup version must lower or equal to current CodeForge version. You can also import
...
func runRestore(ctx context.Context, cmd *cli.Command) error {
	...
	if semverx.Compare(conf.App.Version, "<", backupVersion) {
		log.Fatal("Current CodeForge version is lower than backup version: %s < %s", conf.App.Version, backupVersion)
	}
