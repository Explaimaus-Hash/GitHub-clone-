...
func fail(userMessage, errMessage string, args ...any) {
	_, _ = fmt.Fprintln(os.Stderr, "CodeForge:", userMessage)
...
func runServ(ctx context.Context, cmd *cli.Command) error {
	...
	if conf.SSH.Disabled {
		println("CodeForge: SSH has been disabled")
		return nil
	}
	...
	if sshCmd == "" {
		println("Hi there, You've successfully authenticated, but CodeForge does not provide shell access.")
		println("If this is unexpected, please log in with password and setup CodeForge under another user.")
		return nil
	}
	...
	if requestMode == database.AccessModeWrite || repo.IsPrivate {
		...
		// a server or system using an active deploy key can get read access to all repositories on a CodeForge instance.
